...
def fetch_data(query_val, page_id):
    ...
    for card in card_group:
        ...
        blog = {'mid': mblog['id'],  # 微博id
                ...
                'user_id': str(mblog['user']['id']),  # 用户id
...
def fetch_pages(query_val, start, page_num):
    """抓取关键词多页的数据"""
    mblogs = []
    for page_id in range(start, 1 + page_num + 1):
        try:
    ...
    # 保存到 result.json 文件中
    fp = open('result_{}-{}-{}.json'.format(query_val, start, page_num), 'w', encoding='utf-8')
    json.dump(mblogs, fp, ensure_ascii=False, indent=4)
    print("已保存至 result_{}-{}-{}.json".format(query_val, start, page_num))


if __name__ == '__main__':
    # print(url_template.format('护发', '护发', 101))
    fetch_pages('护发', 0, 100)
